  ...
  return (
    <TiltCard3D className="relative w-full" max={4}>
      <div className="cockpit-glass relative overflow-hidden rounded-2xl px-3 py-4 sm:rounded-3xl sm:px-5 sm:py-5">
        ...
        <div className="relative z-10 grid grid-cols-3 gap-1 divide-x divide-white/[0.06] sm:gap-0">
          {stats.map((s) => (
            <div
              key={s.label}
              className="text-center px-1 py-0.5 sm:px-2 first:pl-0 last:pr-0"
            >
              <p className="text-[8px] font-mono uppercase tracking-[0.18em] text-zinc-500 sm:text-[10px] sm:tracking-widest">
                {s.label}
              </p>
              <p className="mt-0.5 font-display text-xl font-extrabold text-neon-purple [text-shadow:0_0_20px_rgba(191,0,255,0.35)] sm:mt-1 sm:text-3xl lg:text-4xl">
                {s.value}
              </p>
              <p className="text-[10px] text-zinc-500 sm:text-xs">{s.sub}</p>
            </div>
          ))}
        </div>
        <div className="relative z-10 mt-4 border-t border-white/[0.08] pt-4 sm:mt-5 sm:pt-4">
          <div className="flex justify-center">
            <span className="inline-flex max-w-full items-center gap-2 rounded-full border border-neon-lime/25 bg-zinc-950/90 px-2.5 py-1 text-[8px] font-mono font-medium uppercase tracking-[0.16em] text-neon-lime shadow-[0_0_16px_rgba(223,255,0,0.1)] [text-shadow:0_0_6px_rgba(223,255,0,0.2)] sm:px-3.5 sm:py-1.5 sm:text-[10px] sm:tracking-widest">
